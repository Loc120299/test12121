          ...
          <div>
            ...
            <div v-if="paymentInfo.stripePublicKey">
              ...
              <div v-show="paymentMethod === 'VISA'" class="my-5">
                <client-only>
                  <stripe-element-card
                    v-if="paymentInfo.stripePublicKey != ''"
                    ref="elementRef"
                    :pk="paymentInfo.stripePublicKey"
              ...
              <div v-show="paymentMethod === 'PAYPAL'">
                <client-only>
                  <Paypal
                    ...
                    @onPaymentSuccess="onPaymentPaypalSuccess"
                    @onPaymentError="onPaymentPaypalError"
                  />
                </client-only>
              </div>
            </div>
  ...
  methods: {
    ...
    onPayStripe() {
      if (this.products.length === 0) {
        this.$toast.error('You must choose at least a product')
        return
      }
        ...
        .then((resp) => {
          this.$axios
            .$post('/order/stripe/purchase', {
              products: this.convertToPaymentProduct,
